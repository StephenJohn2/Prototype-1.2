...
void Platform::Create_platform(b2World* world_,float x , float y, float width, float height)
{
	...
	// create the ground
	body = world_->CreateBody(&ground_body_def);
	...
	body -> CreateFixture(&ground_shape, 0.0f);
	...
	float ground_x = BOX2D_GFX_POS_X(body->GetPosition().x);
	float ground_y = BOX2D_GFX_POS_Y(body->GetPosition().y);
